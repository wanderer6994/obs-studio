		...
		size_t mix_idx, uint64_t timestamp, uint32_t frames)
{
	...
	for (size_t i = main_mix->inputs.num; i > 0; i--) {
		struct audio_input *main_input = main_mix->inputs.array + (i - 1);
		struct audio_input *streaming_input = streaming_mix->inputs.array + (i - 1);
		struct audio_input *recording_input = recording_mix->inputs.array + (i - 1);

		for (size_t i = 0; i < audio->planes; i++) {
			main_data.data[i] = (uint8_t*)main_mix->buffer[i];
			streaming_data.data[i] = (uint8_t*)streaming_mix->buffer[i];
			recording_data.data[i] = (uint8_t*)recording_mix->buffer[i];
		}
		...
		uint64_t audio_time, uint64_t prev_time)
{
	...
	for (size_t mix_idx = 0; mix_idx < MAX_AUDIO_MIXES; mix_idx++) {
...

		{
			...
			for (size_t i = 0; i < audio->planes; i++)
				streaming_data[mix_idx].data[i] = mix->buffer[i];
		}

		{
			...
			for (size_t i = 0; i < audio->planes; i++)
				recording_data[mix_idx].data[i] = mix->buffer[i];
		}
	}

	/* get new audio data */
	success = audio->input_cb(audio->input_param, prev_time, audio_time,
			&new_ts, active_mixes, main_data, streaming_data, recording_data);
	if (!success)
		return;
		...
		audio_output_callback_t callback, void *param)
{
	...
	if (audio_get_input_idx(audio, mi, callback, param) == DARRAY_INVALID) {
		...
		recording_input.callback = streaming_input.callback = main_input.callback = callback;
		recording_input.param    = streaming_input.param    = main_input.param    = param;

		if (conversion) {
			recording_input.conversion = streaming_input.conversion =
				main_input.conversion = *conversion;
		} else {
			recording_input.conversion.format = streaming_input.conversion.format =
				main_input.conversion.format = audio->info.format;
			recording_input.conversion.speakers = streaming_input.conversion.speakers =
				main_input.conversion.speakers = audio->info.speakers;
